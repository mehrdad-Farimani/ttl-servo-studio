    ...
    def rxPacket(self):
        ...
        while True:
            if self.portHandler.isPacketTimeout():
                result = COMM_RX_CORRUPT if rxpacket else COMM_RX_TIMEOUT
                break
            rxpacket.extend(self.portHandler.readPort(max(0, wait_length - rx_length)))
            ...
            if rx_length >= wait_length:
                ...
                if idx == 0:  # found at the beginning of the packet
                    if (rxpacket[PKT_ID] > 0xFD) or (rxpacket[PKT_LENGTH] < 2) or (rxpacket[PKT_LENGTH] > RXPACKET_MAX_LEN) or (
